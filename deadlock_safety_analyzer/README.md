...
```bash
python src/main.py --input data/sample_state.txt --output outputs/report.json
```

### Buoc 2 (tuy chon): Mo phong tien trinh xin cap them tai nguyen

```bash
python src/main.py --input data/sample_state.txt --request "1,1,0,0"
```
...
```bash
python -m unittest tests/test_banker.py -v
```
...
tung buoc) va `explanation`.

## 8) Kich ban thuyet trinh / demo
...
- Mo `outputs/report.json` de thay cau truc du lieu dau ra.
